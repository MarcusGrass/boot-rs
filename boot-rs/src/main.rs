...
fn boot() -> Result<(), String> {
    ...
    let _ = uefi::system::with_stdout(|output| output.write_str("[boot-rs]: Decrypted kernel file, loading kernel image.\n"));
...
fn read_kernel_data<P: ProtocolPointer + ?Sized>(
    ...
    let hb = uefi::boot::locate_handle_buffer(SearchType::ByProtocol(&P::GUID))
        .map_err(|e| {
            format!("ERROR: Failed to locate handle buffers for SimpleFileSystem: {e:?}")
        })?;
    let to_text_handle =
        uefi::boot::get_handle_for_protocol::<DevicePathToText>()
        .unwrap();
    let to_text_path_protoc = uefi::boot::open_protocol_exclusive::<DevicePathToText>(to_text_handle)
        .unwrap();
    for handle in hb.iter() {
        if let Ok(dev_protoc) = uefi::boot::open_protocol_exclusive::<DevicePath>(*handle)
        {
            ...
            for node in dev_protoc.node_iter() {
                ...
                if let Ok(can_load_fs) = uefi::boot::locate_device_path::<SimpleFileSystem>(&mut dev_ptr)
                {
...
fn decrypt_kernel(buf: &[u8]) -> Result<Vec<u8>, String> {
    for i in 1..4 {
        let key = get_pass()?;
        let _ = uefi::system::with_stdout(|output| output.write_fmt(format_args!("[boot-rs]: Got password, deriving key and attempting decrypt.\n")));
            ...
            Ok(tgt) => {
                ...
                let failed_time = if let (Ok(t0), Ok(t1)) = (t0, t1) {
                    ...
                    if let Some(delta) = get_time_delta_millis(t0, t1) {
                        ...
                        let _ = uefi::system::with_stdout(|output| output.write_fmt(format_args!(
...
fn get_pass() -> Result<String, String> {
    let _ = uefi::system::with_stdout(|output| output.write_str("[boot-rs]: Enter kernel decryption key: \n"));
    let mut decr = String::new();
    loop {
        // Safety:
        // Safe if event is not reused after close (we only oneshot it).

        let Some(key_ready_evt) = uefi::system::with_stdin(|input| input.wait_for_key_event()) else {
            continue;
        };
        ...
        let key = uefi::system::with_stdin(|input| {
            input.read_key()
        })
            .map_err(|e| {
...
fn await_enter() {
    let _ = uefi::system::with_stdout(|output| output.write_str("[boot-rs]: Press enter to exit.\n"));
    let _ = uefi::system::with_stdin(|input| {
        loop {
